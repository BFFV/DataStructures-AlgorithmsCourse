...
class SetSum():
  ...
  def is_valid(self, x, value):
    ...
    if value == 1:
      # El valor de la variable no puede ser superior al valor a sumar
      return self.target >= x.numeric_value
    # Si no se está incluyendo
    else:
      # Lo que queda no puede ser menor a lo que tenemos que sumar
...
if __name__ == "__main__":  
  ...
  if setsum.is_solvable():
    print(setsum)
  else:
    print("No se pudo resolver")

  print(f"Me equivoqué {setsum.undos} veces")
